...
def validate_topological_consistency(base_path: Path, progress_callback=None, layer_filter: str | None = None) -> list[CheckResult]:
    ...
    for idx, (layer_name, real_name) in enumerate(layers_to_process):
        ...
        _progress(pct_base, f"Topological consistency: layer {idx+1}/{total_layers} — {real_name}")
        ...
        for batch_start in range(0, total, BATCH_SIZE):
            ...
            _progress(
                pct_base,
                f"Topological consistency: {real_name} — processing {batch_start}/{total} records..."
            )
            ...
            if len(batch_valid) > 0:
                ...
                HAS_EQUALS = hasattr(shapely, "equals")
                ...
                if mask_inv.any():
                    for geom in geom_arr[mask_inv]:
                        if not HAS_SET_PRECISION:
                            batch_invalid += 1
                            continue
                        try:
                            geom_prec = shapely.set_precision(geom, grid_size=1e-9, mode="valid_output")
                            if not shapely.is_valid(geom_prec):
                                batch_invalid += 1
                                continue
                            if HAS_EQUALS:
                                if shapely.equals(geom, geom_prec):
                                    continue
                                else:
                                    batch_invalid += 1
                            else:
                                if geom.equals(geom_prec):
                                    continue
                                else:
                                    batch_invalid += 1
                        except Exception:
                ...
                if invalid_count > 0:
                    _progress(pct_base, f"Topological consistency: {real_name} — topological error detected, stopping check")
                    del batch_gdf, batch_valid
                    break

            _progress(pct_base, f"Topological consistency: {real_name} — {batch_end}/{total} records checked")
            del batch_gdf, batch_valid

        t_elapsed = _time_mod.time() - t_start
        _progress(pct_base, f"Topological consistency: {real_name} — completed in {t_elapsed:.1f}s")
...
def validate_topological_consistency_detailed(base_path: Path, progress_callback=None, layer_filter: str | None = None, error_limit: int | None = None) -> list[CheckResult]:
    """
    Checks geometric integrity with detail: identifies WHICH features have
    errors and WHAT the error is (reason for invalidity).

    Can only be run on a single layer (layer_filter required).
    Returns CheckResults with individual details per invalid feature.

    Parameters:
        error_limit: maximum number of topological errors to report. None = no limit.
                     Upon reaching the limit, processing stops (early exit).
    """
    results = []
    ...
    pct_base = 65
    _progress(pct_base, f"Detailed topological consistency: {real_name}")
    ...
    for batch_start in range(0, total, BATCH_SIZE):
        ...
        batch_end = min(batch_start + BATCH_SIZE, total)
        _progress(
            pct_base + int((batch_start / total) * 35),
            f"Detailed topological consistency: {real_name} — processing {batch_start}/{total} records..."
        )
        ...
        if batch_null > 0:
            for idx in batch_gdf.index[~batch_notna].tolist():
                fid_global = batch_start + idx
                detailed_errors.append({"fid": fid_global, "error": "Null geometry (NULL)", "wkt": ""})
        ...
        if batch_empty > 0:
            mask_empty = batch_notna & ~batch_not_empty
            for idx in batch_gdf.index[mask_empty].tolist():
                fid_global = batch_start + idx
                geom = batch_gdf.geometry.iloc[idx]
                wkt_str = geom.wkt if geom is not None else ""
                detailed_errors.append({"fid": fid_global, "error": "Empty geometry (EMPTY)", "wkt": wkt_str})
        ...
        if len(batch_valid) > 0:
            ...
            if mask_inv.any():
                ...
                suspect_geoms = geom_arr[mask_inv]

                for idx, geom in zip(suspect_indices, suspect_geoms):
                    fid_global = batch_start + idx
                    ...
                    if not HAS_SET_PRECISION:
                        ...
                        try:
                            geom_prec = shapely.set_precision(geom, grid_size=1e-9, mode="valid_output")
                            if not shapely.is_valid(geom_prec):
                                reason = shapely.is_valid_reason(geom_prec) if HAS_VECTOR_API else str(geom_prec.is_valid)
                                detailed_errors.append({"fid": fid_global, "error": str(reason), "wkt": geom.wkt if geom is not None else ""})
                            else:
                                equal = shapely.equals(geom, geom_prec) if HAS_EQUALS else geom.equals(geom_prec)
                                if not equal:
                                    reason = shapely.is_valid_reason(geom) if HAS_VECTOR_API else str(geom.is_valid)
                                    detailed_errors.append({"fid": fid_global, "error": str(reason), "wkt": geom.wkt if geom is not None else ""})
                        except Exception as exc:
                            detailed_errors.append({"fid": fid_global, "error": f"Error validating geometry: {exc}", "wkt": geom.wkt if geom is not None else ""})
                    ...
                    if error_limit is not None and topo_errors_so_far >= error_limit:
                        _progress(
                            ...
                            f"Detailed topological consistency: {real_name} — "
                            f"limit of {error_limit} error(s) reached, stopping"
                        )
                        limit_reached = True
                        break
        ...
        _progress(
            pct_base + int((batch_end / total) * 35),
            f"Detailed topological consistency: {real_name} — {batch_end}/{total} records checked"
        )
        del batch_gdf, batch_valid

    t_elapsed = _time_mod.time() - t_start
    _progress(100, f"Detailed topological consistency: {real_name} — completed in {t_elapsed:.1f}s")
    ...
    topological_errors = [e for e in detailed_errors if "null" not in e["error"].lower() and "empty" not in e["error"].lower()]
    if topological_errors:
        limit_suffix = (
        ...
        results.append(CheckResult(
            name="Topological errors",
            status=Status.NON_CONFORMANT,
            details=f"{len(topological_errors)} geometry(ies) with topological errors{limit_suffix}",
            category="topological",
            layer=layer_filter,
        ))
        for error in topological_errors:
            results.append(CheckResult(
        ...
        results.append(CheckResult(name="Topological errors", status=Status.CONFORMANT, details="No topological errors detected", category="topological", layer=layer_filter))

    null_empty_errors = [e for e in detailed_errors if "null" in e["error"].lower() or "empty" in e["error"].lower()]
...
def validate_dataset(base_path: str | Path, progress_callback=None, layer_filter: str | None = None, detailed: bool = False, error_limit: int | None = None) -> ValidationReport:
    """
    ...
    Parameters:
        ...
        detailed: if True, uses detailed topological validation (identifies features and reasons)
        error_limit: maximum number of topological errors to report in detailed validation
                     (None = no limit)
    ...
    if base_path.suffix.lower() == ".zip" and base_path.is_file():
        _progress(0, "Extracting GPKG from ZIP archive...")
        extracted = extract_gpkg_from_zip(base_path)
        if extracted and extracted.exists():
            _temp_dir = extracted.parent
            base_path = extracted
            _progress(5, f"GPKG extracted: {extracted.name}")
        else:
    ...
    _progress(0, "Checking format...")
    format_checks = validate_format(base_path)
    report.checks.extend(format_checks)
    _progress(10, f"Format checked ({len(format_checks)} checks)")

    if any(c.status == Status.NON_CONFORMANT for c in format_checks):
        _progress(100, "Completed (invalid format)")
        report.log = _log_entries
        return report

    # 2. Conceptual consistency (10-40%)
    _progress(15, "Checking conceptual consistency...")
    report.checks.extend(validate_conceptual_consistency(base_path, layer_filter=layer_filter))
    _progress(40, "Conceptual consistency checked")

    # 3. Domain consistency (40-60%)
    _progress(45, "Checking domain consistency...")
    report.checks.extend(validate_domain_consistency(base_path, layer_filter=layer_filter))
    _progress(60, "Domain consistency checked")

    # 4. Topological consistency (60-100%)
    _progress(65, "Checking topological consistency...")
    if detailed and layer_filter:
        report.checks.extend(validate_topological_consistency_detailed(base_path, progress_callback=progress_callback, layer_filter=layer_filter, error_limit=error_limit))
    else:
        report.checks.extend(validate_topological_consistency(base_path, progress_callback=progress_callback, layer_filter=layer_filter))
...
def export_report_pdf(report: ValidationReport, output_path: str | Path) -> Path:
    """
...
def _generate_report_html(report: ValidationReport) -> str:
    """
    Generates the formatted HTML content of the report in GOV.BR DS style.

    Structure:
    - Header with dark background (Blue Warm Vivid 90) + SFB/SICAR logos
    ...
    BORDER = "#CCCCCC"
    BORDER_LIGHT = "#E6E6E6"
    FONT = "Rawline, Roboto, Arial, sans-serif"
    ...
    logo_sicar_html = ""
    if logo_sfb_b64:
        logo_sfb_html = f"<img src='data:{logo_sfb_mime};base64,{logo_sfb_b64}' height='36' style='margin-right: 8px;'>"
    if logo_sicar_b64:
        logo_sicar_html = f"<img src='data:{logo_sicar_mime};base64,{logo_sicar_b64}' height='30' style='margin-left: 8px;'>"

    if report.passed and not report.has_warnings:
        badge_bg, badge_border, badge_text, badge_color = SUCCESS_BG, SUCCESS, "CONFORMANT", SUCCESS
    elif report.passed and report.has_warnings:
        badge_bg, badge_border, badge_text, badge_color = WARNING_BG, WARNING, "CONFORMANT WITH WARNINGS", WARNING
    else:
        badge_bg, badge_border, badge_text, badge_color = ERROR_BG, ERROR, "NON-CONFORMANT", ERROR
    ...
    html_parts.append(
        ...
        f"<span style='font-size: 11pt; font-weight: 600; color: {TEXT_PRIMARY};'>SICAR AD — Validate Environmental Dataset Structure</span><br>"
        f"<span style='font-size: 8pt; color: {TEXT_SECONDARY};'>Conformance Assessment based on ISO 19157:2013</span>"
        f"</td>"
    ...
    html_parts.append(
        ...
        f"<tr><td><b>Validation date:</b> {date_str}</td>"
        f"<td style='text-align: right;'><b>Source:</b> {file_name}</td></tr></table>"
    )

    html_parts.append(
        f"<div style='background: {badge_bg}; border-left: 4px solid {badge_border}; padding: 10px 16px; margin-bottom: 16px;'>"
        f"<span style='font-size: 8pt; color: {TEXT_SECONDARY}; font-weight: 600;'>Validation result: </span>"
        f"<span style='font-size: 8pt; font-weight: 700; color: {badge_color};'>{badge_text}</span><br>"
        f"<span style='font-size: 8pt; color: {TEXT_SECONDARY};'><b>Summary:</b> {report.summary()}</span></div>"
    )

    html_parts.append(
        f"<h4 style='color: {INTERACTIVE}; font-size: 10pt; font-weight: 600; margin: 12px 0 6px 0; border-bottom: 2px solid {INTERACTIVE}; padding-bottom: 4px;'>Summary by Category</h4>"
        ...
        f"<th style='text-align: left; border-bottom: 2px solid {BORDER}; padding: 6px 8px;'>Category</th>"
        f"<th style='text-align: left; border-bottom: 2px solid {BORDER}; padding: 6px 8px;'>Status</th>"
        f"<th style='text-align: left; border-bottom: 2px solid {BORDER}; padding: 6px 8px;'>Result</th></tr>"
    )

    for cat_name, info in report.summary_by_category().items():
        if info["status"] == Status.CONFORMANT:
            st_html = f"<span style='color: {SUCCESS}; font-weight: 600;'>✓ Conformant</span>"
            row_bg = ""
        elif info["status"] == Status.WARNING:
            st_html = f"<span style='color: {WARNING}; font-weight: 600;'>⚠ Warning</span>"
            row_bg = f" style='background: {WARNING_BG};'"
        else:
            st_html = f"<span style='color: {ERROR}; font-weight: 600;'>✗ Non-conformant</span>"
            row_bg = f" style='background: {ERROR_BG};'"
        html_parts.append(
            f"<tr{row_bg}><td style='padding: 6px 8px; border-bottom: 1px solid {BORDER_LIGHT};'>{cat_name}</td>"
            f"<td style='padding: 6px 8px; border-bottom: 1px solid {BORDER_LIGHT};'>{st_html}</td>"
            f"<td style='padding: 6px 8px; border-bottom: 1px solid {BORDER_LIGHT};'>{info['conformant']}/{info['total']} conformant</td></tr>"
        )
    ...
    try:
        ...
        html_parts.append(
            f"<h4 style='color: {INTERACTIVE}; font-size: 10pt; font-weight: 600; margin: 12px 0 6px 0; border-bottom: 2px solid {INTERACTIVE}; padding-bottom: 4px;'>Layers found in dataset</h4>"
    ...
    if non_conformant:
        html_parts.append(
            ...
            f"<b style='color: {ERROR}; font-size: 9pt;'>Non-conformant items ({len(non_conformant)})</b>"
        ...
        for check in non_conformant:
            ...
            html_parts.append(f"<li><b>{check.name}</b>{layer_txt}: {check.details}</li>")
    ...
    if warnings:
        html_parts.append(
            ...
            f"<b style='color: {WARNING}; font-size: 9pt;'>Warning items ({len(warnings)})</b>"
            f"<ul style='font-size: 7pt; margin: 4px 0; padding-left: 16px;'>"
        )
    ...
    for cat_name, checks in categories:
        ...
        html_parts.append(
            ...
            f"<tr style='background: {SURFACE_ALT};'>"
            f"<th style='text-align: left; border-bottom: 2px solid {BORDER}; padding: 5px 6px;' width='20%'>Layer</th>"
            f"<th style='text-align: left; border-bottom: 2px solid {BORDER}; padding: 5px 6px;' width='16%'>Check</th>"
            f"<th style='text-align: left; border-bottom: 2px solid {BORDER}; padding: 5px 6px;' width='14%'>Status</th>"
            f"<th style='text-align: left; border-bottom: 2px solid {BORDER}; padding: 5px 6px;' width='50%'>Details</th></tr>"
        )
        for idx, check in enumerate(checks):
            row_bg = f" style='background: {SURFACE_ALT};'" if idx % 2 == 1 else ""
            if check.status == Status.CONFORMANT:
                st_td = f"<td style='color: {SUCCESS}; font-weight: 600; padding: 4px 6px; border-bottom: 1px solid {BORDER_LIGHT};'>✓ Conformant</td>"
            elif check.status == Status.WARNING:
                st_td = f"<td style='color: {WARNING}; font-weight: 600; padding: 4px 6px; border-bottom: 1px solid {BORDER_LIGHT};'>⚠ Warning</td>"
            else:
                st_td = f"<td style='color: {ERROR}; font-weight: 600; padding: 4px 6px; border-bottom: 1px solid {BORDER_LIGHT};'>✗ Non-conformant</td>"
            html_parts.append(
                f"<tr{row_bg}>"
                f"<td style='padding: 4px 6px; border-bottom: 1px solid {BORDER_LIGHT};'>{check.layer}</td>"
                f"<td style='padding: 4px 6px; border-bottom: 1px solid {BORDER_LIGHT};'>{check.name}</td>"
                f"{st_td}"
                f"<td style='padding: 4px 6px; border-bottom: 1px solid {BORDER_LIGHT};'>{check.details}</td></tr>"
            )
        html_parts.append("</table>")
    ...
    html_parts.append(
        ...
        f"Brazilian Forest Service — SFB &nbsp;|&nbsp; National Rural Environmental Registry — SICAR<br>"
        f"Report automatically generated by SICAR AD plugin — Validate Environmental Dataset Structure<br>"
        f"Digital Government Standard · gov.br"
        f"</div>"
    )
